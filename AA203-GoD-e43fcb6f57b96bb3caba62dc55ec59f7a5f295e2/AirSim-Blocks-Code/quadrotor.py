...
class Quadrotor():
    ...
    def __init__(self, m = 1, kf = 1e-5, km = 1e-5, Jx = 1, Jy = 1, Jz = 2, 
        ...
        (self.gxy, self.gz, self.gxyv, self.gzv, self.gxyr, self.gzr, 
                self.gxyw, self.gzw) = (1, 1, 1, 1, 1, 1, 1, 1)
    ...
    def setGains(self, gxy, gz, gxyv, gzv, gxyr, gzr, gxyw, gzw):
        """
        set the PD gains
        """
        (self.gxy, self.gz, self.gxyv, self.gzv, self.gxyr, self.gzr, 
            self.gxyw, self.gzw) = (gxy, gz, gxyv, gzv, gxyr, gzr, gxyw, gzw)
    ...
    def getStateAtNomTime(self, traj, t):
        ...
        omega_nw = np.array([out_state[9],out_state[10],out_state[11]])

        u1_dot = - mass * np.dot(jer_t, zn)
        u1_ddot = -np.dot(
            (mass * sna_t + np.cross(omega_nw, np.cross(omega_nw, zn))),
            zn)

        h_alpha = (-1.0/u0_ff) * (
            mass * sna_t + u1_ddot * zn + \
                2.0 * u1_dot * np.cross(omega_nw,zn) + \
                    np.cross(omega_nw, np.cross(omega_nw, zn)))
        ...
        # possibly minus sign
        u_vec = J @ omega_dot_nw + np.cross(omega_nw, J @ omega_nw)
        nom_control = np.zeros((4,))
        nom_control[0] = u0_ff
        nom_control[1:4] = u_vec
        return out_state.copy(), nom_control.copy()

    def setStateAtNomTime(self, traj, t):
        '''
        Sets the quadrotor state to the nominal path state at time t
        '''
        out_state, out_control = self.getStateAtNomTime(traj, t)
        self.setState(out_state)
        return out_state.copy(), out_control.copy()


    def PDController(self, traj, airsim = False):
        """
        ...
        # gains
        # only need to edit gxy, gz, gzyv, gzv for airsim case
...
class Trajectory():
    ...
    def sigma(self, t, order = 0):
        j = min(np.searchsorted(self.t_kf,t), self.sigma_coeffs.shape[1]-1)
        sigma_arr = [None] * 4
        sigma_ret = [None] * 4
        for idx in range(4):
